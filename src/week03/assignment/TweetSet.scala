...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def union(that: TweetSet): TweetSet =
    this.filterAcc((x: Tweet) => !that.contains(x), that)

  override def mostRetweeted: Tweet =
    try {
      val a: Tweet = left.union(right).mostRetweeted
      if ( elem.retweets >= a.retweets ) elem
      else a
    } catch {
...
class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  override def toString(): String = "(" + head.user + "," + head.retweets + ") - " + tail
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    def p(x: Tweet):Boolean = !google.forall( (y: String) => !x.text.contains(y) )
    TweetReader.allTweets.filter(p)
  }
  lazy val appleTweets: TweetSet = {
    def p(x: Tweet):Boolean = !apple.forall( (y: String) => !x.text.contains(y) )
    TweetReader.allTweets.filter(p)
  }
